...
def positionLogicPlan(problem):
    ...
    # Backtracking is bad, so we keep track of our explored states.
    exploredStates = {}
    exploredStates[initialState] = time_step

    # Pushing in initial states to process.
    possibleStates = []
    ...
    while 1:
        # Goal check: test if we are at the goal position.
        goal_check = list(kb)
        ...
        # Make sure we take one action per time step.
        curActions = []
        ...
        # Adding constraints for impossible coordinates.
        impossibleCoords = []
        for x in xrange(1, width + 1):
            for y in xrange(1, height + 1):
...
def foodLogicPlan(problem):
    ...
    # Setting constraint that you can only occupy one initial position.
    ...
    while 1:
        # Goal check: test if all spaces with food have been occupied at some time step.
        goal_check = list(kb)
        total_food = []
        for pos in foodList:
            food_check = []
        ...
        for state in possible:
            ...
            # Make sure we don't perform redundant processing.
...
def foodGhostLogicPlan(problem):
    ...
    Note that STOP is not an available action.
    """
    # Function to determine ghost position.
    ...
    # Ghosts are stored in a tuple (start_position, start_direction), which can then be used to
    # calculate ghost position at any time.
    ghostPositions = []
    ...
    while 1:
        ...
        # Determine current ghost locations depending on starting location and directions.
        ghostLocations = []
        ...
        for state in possible:
            ...
            # Forget any positions that will get us killed.
