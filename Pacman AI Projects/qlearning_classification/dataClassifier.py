...
def enhancedFeatureExtractorDigit(datum):
    ...
    Tries to find the number of connected whitespace regions by:

    Checking for whitespace on individual pixels.
    If whitespace is found, check neighbors for additional whitespace within
    the region and mark it as visited, as we don't want to mark this as an
    additional whitespace regions.
    This will give us the total number of whitespace regions.
    ##
    """
...
def enhancedPacmanFeatures(state, action):
    ...
    successor_pacman_position = successor_state.getPacmanPosition()
    ghostList = successor_state.getGhostPositions()
    foodGrid = successor_state.getFood()
    capsuleList = successor_state.getCapsules()

    # Are ghosts closer to your current position or successor position?
    min1 = 999999999
    min2 = 999999999
    for ghost_position in ghostList:
        if util.manhattanDistance(ghost_position, successor_pacman_position) < min1:
            min1 = util.manhattanDistance(ghost_position, successor_pacman_position)
        if util.manhattanDistance(ghost_position, pacman_position) < min2:
            min2 = util.manhattanDistance(ghost_position, pacman_position)

    if (min1 < min2):
        features['ghostsCloser'] = 1
    else:
        features['ghostsCloser'] = 0

    # Are there pellets close by?
    pellet_proximity = 0
    for food in foodGrid.asList():
        pellet_proximity += 1 / util.manhattanDistance(successor_pacman_position, food)

    features['pelletProximity'] = pellet_proximity

    # Are there food capsules?
    if (capsuleList):
        features['capsuleCount'] = 1
    else:
        features['capsuleCount'] = 0        

    # Do we have a higher score?
    if (successor_state.getScore() > state.getScore()):
        features['increaseScore'] = 1
    else:
        features['increaseScore'] = 0
