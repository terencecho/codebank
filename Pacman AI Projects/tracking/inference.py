...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        # if ghost caught by pacman, place it in its prison cell
        #
        # otherwise, evaluate the noisy distance probability for each
        # legal action and place in new belief state
        if noisyDistance == None:
            newBeliefs[self.getJailPosition()] = 1.0
        else:
            for p in self.legalPositions:
                probNoisyDist = emissionModel[util.manhattanDistance(p, pacmanPosition)]
                if probNoisyDist > 0:
                    newBeliefs[p] += probNoisyDist * self.beliefs[p]

        # normalize and update the current belief state
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        # update belief state given timestep
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        for i in range(numParticles-1):
            if i >= len(legal):
                i = i % len(legal)
            particles.append(legal[i])

        # update the current particles state
    ...
    def observe(self, observation, gameState):
        ...
        # if ghost caught by pacman, place it in its prison cell
        #
        # otherwise, evaluate the distance and place in new belief state     
        if noisyDistance == None:
            newBeliefs[self.getJailPosition()] = 1.0
        else:
            for p in self.particles:
                newBeliefs[p] += emissionModel[util.manhattanDistance(p, pacmanPosition)]

        # normalize and update the current belief state
        newBeliefs.normalize()
        self.beliefs = newBeliefs

        # if all particles received 0 weight, we should recreate the
        # prior distribution by calling initializeUniformly
        #
        # otherwise, update particle distribution with sampled belief
        # distribution
    ...
    def elapseTime(self, gameState):
        ...
        # update belief state given sample generated from new distribution
        for p in self.particles:
    ...
    def getBeliefDistribution(self):
        ...
        # take list of particles and convert into a belief distribution
        for p in self.particles:
            belief[p] += 1
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        # get permutations and shuffle to ensure even placement of particles
        permutations = list(itertools.product(legal, repeat=numGhosts))
        random.shuffle(particles)

        # distribute particles evenly across evaluated permutations
        for i in range(numParticles-1):
            if i >= len(permutations):
                i = i % len(permutations)
            particles.append(permutations[i])
    ...
    def observeState(self, gameState):
        ...
        newBeliefs = util.Counter()
        ghostInJail = util.Counter()

        # resample set of particles using noisy probabilities 
        #
        # if ghost caught by pacman, place it in its prison cell
        for i in range(numGhosts):
            if noisyDistances[i] == None:
                ghostInJail[self.getJailPosition(i)] = 1.0

        for p in self.particles:
            for g in range(numGhosts):
                if self.getJailPosition(g) in ghostInJail:
                    p = self.getParticleWithGhostInJail(p, g)

            belief = 1

            for g in range(numGhosts):
                if self.getJailPosition(g) not in ghostInJail:
                    belief *= emissionModels[g][util.manhattanDistance(p[g], pacmanPosition)]
            newBeliefs[p] += belief
    ...
    def elapseTime(self, gameState):
        ...
        for oldParticle in self.particles:
            ...
            # draw a new position for each ghost given the position of all
            # ghosts at the previous time step
