...
public class KVClientTest {
    ...
    @Test
    public void testDelFail() throws Exception {
        setupSocketSuccess();
        when(msg.getMsgType()).thenReturn(RESP);
        when(msg.getMessage()).thenReturn(ERROR_NO_SUCH_KEY);
        try {
    ...
    @Test
    public void testBadPut() throws Exception {
        setupSocketSuccess();
    	try {
    		client.put(null, "Supvalue");
    		fail("Cannot put null key.");
    	} catch (KVException e) {
    		assertEquals(ERROR_INVALID_KEY, e.getMessage());
    	}
    	
    	try {
    		client.put("Supkey", null);
    		fail("Cannot put null value.");
    	} catch (KVException e) {
    ...
    @Test
    public void testBadDel() throws Exception {
        setupSocketSuccess();
    	try {
    		client.del(null);
