...
public class TPCMurderDeathKillStud {
    ...
    KVMessage delify = new KVMessage(KVConstants.DEL_REQ);
    ...
    public void setUp() throws Exception {
        ...
        delify.setKey("6666666666666666667");
    ...
    protected void startMockSlave(long slaveID, int deathOption) throws Exception {
        ...
        case 2:
        	doAnswer(dieAfterLog).when(log).appendAndFlush(argThat(new isDel())); //kill in phase 1 after log on del
            break;
            
        case 3:
        	doAnswer(dieAfterLog).when(log).appendAndFlush(argThat(new isPutDel2())); //kill in phase 2 commit
        	break;
    ...
    protected void necromancy(long slaveID, String oldLog) throws Exception {
        //System.out.println("In necromancy");
        ...
        //System.out.println(Long.toString(slaveID));
    }
    ...
    public void testDelDeathAfterLog(){
        ...
        try{
        	...
        	fail("Put request should not fail.");
        }
        try{
        ...
        try {
        	assertEquals("GOBEARS", slave1.get(KEY1));
        } catch (KVException e) {
        	fail("Key should not have been deleted.");
        }

        //Verify log integrity by deleting a key successfully, then killing and rebuilding slave.
        try{
            master.handleTPCRequest(verify,true);
            assertTrue(slave1.get("6666666666666666667").equals("demolition man"));
            verify(spyLog, atLeast(2)).appendAndFlush((KVMessage) anyObject());
        ...
        try {
        	master.handleTPCRequest(delify, false);
        } catch (KVException e) {
        	fail("Del on live slave shouldn't fail");
        }

        try {necromancy(SLAVE1, LOG);} catch (Exception e) {fail("Could not rebuild slave.");}
        checkBuild();
        try {
        	slave1.get("6666666666666666667");
        ...
        try {
        	assertTrue(slave1.get(KEY1).equals("GOBEARS"));
        } catch (KVException e) {
        	fail("Server not properly rebuilt.");
        }
