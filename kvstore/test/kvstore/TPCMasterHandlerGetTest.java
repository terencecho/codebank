...
public class TPCMasterHandlerGetTest {
    ...
    @Category(AG_PROJ4_CODE.class)
    @AGTestDetails(points = 1,
        desc = "Test if they can succeed to get one value using Handler")
    public void testGetSuccess() throws KVException {
        setupSocketSuccess();

        masterHandler.kvServer.put("key","success");
        ...
        KVMessage check = new KVMessage(sock3);
        assertEquals(RESP, check.getMsgType());
        assertEquals("success", check.getValue());
    }
